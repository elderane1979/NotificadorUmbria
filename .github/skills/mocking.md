...
- Preferir en los tests asignar valores a las propiedades del mock a través de la instancia devuelta (mock.Object.Prop = valor) en lugar de usar SetupGet/SetupProperty dentro de los tests. Los setups por defecto deben residir en las clases Mock concretas.
- Crear un helper CreateService/GetService en los tests que construya el SUT y acepte parámetros opcionales para inyectar mocks/implementaciones específicas. Por defecto este helper debe usar MockFactory.GetMock<T>().
